...
is_sax <- c(rep(1, dim(type.msax.accuracy)[1]), rep(0, dim(type.msax.accuracy)[1]))
...
acc_plot <-
    ...
    geom_boxplot() +
    labs(title = paste0("Accuracy ", cor(data$is_sax, data$precision, use = "complete.obs")))
...
prec_plot <-
    ...
    geom_boxplot() +
    labs(title = paste0("Precision ", cor(data$is_sax, data$precision, use = "complete.obs")))
