...
\TODO{mention the low number of parameters as a strength}
\TODO{mention and introduce ssax, dsax, msax and what they mean}
...
followed by the MSAX representation, and then the HOT SAX algorithm.\par
...
equla-length SAX representations is defined.
...
The parameter $w$ is one only two parameters that determine the SAX
representation. A low number of parameters can be a great advantage in time
series analysis because it is simpler to optimize few, understandable parameter
compared to many unintuitive parameters.
\begin{figure}[h]
    \centering
...
The resulting time series representation has an even lower complexity than
...
\begin{itemize}                                                                 
    ...
    \item the subsequence length $m$ that determines HOT SAX and HOT MSAX;
    \item the variable $k$ indicating how many discords should be found.
\end{itemize}                                                                   
These parameters determine all actions the program performs afterward. 

\paragaph{Transformation to SAX and MSAX}

In the second step, the ECGs are classified using HOT SAX and HOT MSAX.
First, a CSV file containing the ECG data and annotations is loaded into
...
programming language's ecosystem of libraries, the formulas and algorithms can 
be easily translated into code.\par

The SAX representation is applied to each of the ECG leads individually, while 
MSAX is applied as designed to both at once. HOT SAX and HOT MSAX comprise the 
next step. 

For HOT MSAX, the process is performed using the MSAX representation 
and distance measure. The method returns a list of distances as well as a list 
of indices that indicate which PAA segment has which distance. Depending on the 
parameter $k$, only the top $k$ of these discords are returned. These results 
are then added to the respective PAA segments in the ECG data frame, adding 
both the MSAX distance of the segment as well as a binary indicator of whether 
or not the segment was detected as a discord.\par

\paragraph{

For HOT SAX the process slightly different.
Because SAX is a univariate representation, it cannot be directly applied to
a bivariate ECG. Thus, the SAX representation is applied to each lead of the 
ECG separately and then HOT SAX is performed for the representation of each
lead. 
Each set of results is, like in HOT MSAX, a list of indices of PAA segments and 
a list of their distances. Each sets of results is also added to the ECG data 
frame. This time the detection indicator is quaternary, it represents no 
detection, detection on the first lead, detection on the second lead, or 
detection on both leads. This enables the analysis of two different versions of
HOT SAX. The first one, Single SAX (S-SAX), considers only those indicated PAA
segments that were indicated by a single method or by both methods. This
analysis is then repeated again to get the other S-SAX results. Both S-SAX data
sets can now be combined
