...
\TODO{call it HOT MSAX throughout}
...
representation? \TODO{fix this and make congruent with hypothesis}. Below,
a short introduction to time series will be given. Following that, the SAX
representation will be discussed, followed by the MSAX reresentation and then
the HOT SAX algorithm.\par
...
\begin{table}[h]
    ...
    \end{tabular}
\end{table}
For example, if $a=5$, the $MINDIST$ between ``a" and ``a" is 0, just 
...
below can be expressed as the series of symbols ``cCcCbC \textbf{dD}bCbC bCbBbA 
bBcCcC cBcBcB cBcBbB". In the MSAX representation, the features of the raw data
...
\TODO{try to make this more understandable. introduce inner and outer loop verb
env?}
...
require $m^2$ operations and be equal \mycite{keogh2003}.\par
...
series in descending order by their distance to their neighbors. The authors
approximate this by taking each SAX subsegment and insert them into an array,
counting how many times each unique time series occurs. By sorting this array
...
subsequences that are identical to a given one. So, if one as the subsegment 
``abc", it is possible to find all other locations of the ``abc" subsegment in
the time series using this tree. \figref{08:fig:hot-sax} (quoted from
\mycite{keogh2005}) provides a visual representation of the HOT SAX process.
\begin{figure}[t]
    \centering
...
research's discord discovery method. While HOT SAX was designed to work with
...
be used with HOT SAX. Doing this expands HOT SAX to multivariate time series,
as MSAX can represent those. Using HOT SAX with the MSAX representation is
novel and a contribution of this research.

\TODO{subsection on HOT MSAX}
...
\end{itemize}                                                                   
\TODO{introduce s-sax, d-sax, msax}
