...
that are recorded continuously over a certain interval \mycite{brockwell2016}. 
Time series are used in many disciplines to record information on time-dependent 
...
\begin{enumerate}
    ...
    \item data adaptive,
    \item \label{07-dr-methods:03} model-based, and
    \item data dictated.
\end{enumerate}
...
the Piecewise Aggregate Approximation (PAA) \mycite{aghabozorgi2015}.
...
Approximation, Piecewise Linear Approximation, and Piecewise Constant 
Approximation \mycite{aghabozorgi2015}.

\subsubsection{Model-based representation}

Model based methods use stochastic methods such as Markov Models and Hidden 
Markov Models, and the Auto-Regressive Moving Average (ARMA) 
\mycite{aghabozorgi2015}.

\subsubsection{Data dictated representation}

Data dictated methods derive their compression ratios from the data 
automatically, the most common form of this method is Clipped 
\mycite{aghabozorgi2015}.

\TODO{create some type of transition here}

\subsubsection{The SAX representation}\label{07:section_sax}

\TODO{add some paragraphs to this mess}\\

A particular dimension reduction method is SAX. Introduced by 
\citeauthor{lin2003}, SAX is a symbolic time series representation method. 
...
mining and many researchers have attempted to modify and improve it. The SAX
distance measure has been improved to include the standard deviation
...
segments of a time series \mycite{keogh2005}. SAX-ARM combines the SAX
representation with association rule mining (identifying rules and implications
...
each segment to detect ECG anomalies, achieving a recall value of 98\%. 

\subsection{ECGs and ECG Analysis}

In the year 1786, Luigi Galvani noted the electrical activity in muscles and in
1842, Carlo Matteucci showed the electrical activity of a frog's heartbeat. In
1901, Willem Einthoven first created an ECG recording of a human heartbeat and
received the Nobel Prize in medicine in medicine in 1924 for his invention of
the electrocariograph. As a result of further development, the American Heart
Association published a standardization of the 12-lead ECG that we know today
\mycite{alghatrif2012}. 
