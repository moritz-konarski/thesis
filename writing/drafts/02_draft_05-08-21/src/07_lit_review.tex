...
\TODO{make this a more complete introduction, give a verbal table of contents
with reference to the different sections}
...
\subsubsection{Data dictated representation}\label{07:reduction-methods:01}
...
\subsubsection{Non-data adaptive representation}\label{07:reduction-methods:02}
...
and the Auto-Regressive Moving Average (ARMA) \mycite{aghabozorgi2015}. A HMM
was used in \mycite{panuccio2002} to cluster electroencephalograph recordings 
(measuring the brain's electrical activity). It was found that their methods
...
occurrences of this arrhythmia together \mycite{corduas2008}.

\subsubsection{Data adaptive representation}\label{07:reduction-methods:04}
...
this method, please refer to \TODO{refer to the methodology section of HOT 
SAX}.\par
...
their individual time series and those results were combined. Their analysis 
...
significantly more accurate, in 2 of the MSAX was more accuarate, and in the
...
thoroughly presented in \TODO{refer to methods section}.
\TODO{mention this in the methodology section as supporting information}
...
give off an electrical pulse, initiates the heart beat. Its pulse is conducted
...
\figref{07:fig:ecg_ann} shows a ECG wave of a single heartbeat from record 103
...
diagnosis that can be performed using an ECG is vast. The AHA lists 88 
...
and ischaemic heart disease. Cardiac arrhythmia is a variation of the heart
...
the interval between the QRS complexes. Ischaemic heart disease is the main 
cause of death world-wide \mycite{nowbar2019}. Ischemic heart disease is 
characterized by 
restricted blood flow to an area of the heart, causing it to not receive 
enough blood and oxygen. Blood flow restriction is caused by a blockage (or 
narrowing) in a blood vessel supplying the heart muscle. An artery can be 
blocked by a blood clot, but the most common cause is plaque buildup, called 
atherosclerosis. If the circulation to the heart is completely blocked, the 
cells in the heart muscle begin to die. This is called myocardial infarction, 
more commonly known as a heart attack. The deprivation of oxygen the heart 
experiences leads to the characteristic chest pain commonly associated with 
heart attacks \mycite{ihd2010}. In an ECG, ischaemic heart disease can be
...
discussed in section \label{07:time-series-intro}, multivariate time series are
...
the digitizing of paper-based ECG records. For either processes, the AHA
...
precisely targeted and the clean signal reconstructed afterwards. Choosing 
...
sections \ref{07:reduction-methods:01}--\ref{07:reduction-methods:04} for 
...
Transform (see section \ref{07:reduction-methods:02}) provides a way of 
...
the time series
representation methods discussed in sections 
\ref{07:reduction-methods:01}--\ref{07:reduction-methods:04} can be
applied.\par

Finally, the extracted features can be classified; this is stage 
\ref{07:comp-ecg-an:05}. In this stage judgements are made based on the 
...
include the k-nearest-neighbors model which classifies points into groups
but which is very expensive to calculate for high-dimensional data. Support 
vector machines are used for pattern recognition and are able to work with 
...
fed raw data perform steps 
\ref{07:reduction-methods:01}--\ref{07:reduction-methods:05} internally, as a 
...
A very important element of computarized ECG analysis is the training data.
...
individual heart beats. ECG databases fulfill these requirements. One of the
...
subects, each being 2 hours long and containing two leads. This database is
focused on the ST segment and the T wave (hence the name) and thus focuses on
ischaemia detection. One of the most used databases in the literature is the 
