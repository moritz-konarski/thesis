...
\TODO{fix verbatim environments, make them inline}
...
corresponds to 2,640 unique sets of parameters. For the parameter $k$ the
values $\{-1,  25,  50, 100, 150, 200, 300, 500\}$ were used. The value $k=-1$
corresponds to the situation that all detected discords are returned,
regardless of how many are detected. The number of PAA segments ranged $w$ was
$\{ 2,  3,  4,  5, 12, 20, 30, 40, 60 \}$. The subsequence lengths considered
were also in this set, and it was made sure that the subsequence length $m$ 
evenly divides the number of PAA segments $w$. The alphabet sizes $a$ were part 
of the
set $\{ 4,  5,  6,  7,  8,  9, 10, 12, 14, 17, 20\}$. These sets of parameters
\begin{itemize}
...
\end{itemize}
can be combined into 2640 unique combinations that were analyzed. For HOT SAX
with MSAX, the highest recall value was 99.98\% on average across all 48 ECGs
for the parameters $k=-1,\,w=60,\,m=60,\,a=20$. These results seem to
indicate a few important things:
\begin{itemize}
...
subsequence lengths smaller than the interval of 1 second. This dataset will
have more diversity when it comes to the parameters and thus that dataset will
be used for analysis.\TODO{expand this statement to SAX as well, make
subsections}

\subsection{Data Generation 2}

The second round of data generation yielded 238,464 files corresponding to
4,968 unique parameter combinations, each for 48 ECGs. The parameters used in
this are: $k\in \{-1,  25,  50,  75, 100, 150, 175, 200, 300\}$, $w \in \{  2,
3,   4,   5,   6,   8,   9,  10,  12,  15,  18,  20,  24,  30,  36,  40,  45,
60,  72,  90, 120, 180, 360 \}$, and $a \in \{ 2,  3,  4 , 5,
...
Then, these values were sorted by their precision. This is meant to represent
a compromise between recall and precision. The top 10 results showed parameter
